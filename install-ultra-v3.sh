...
detect_hardware() {
    ...
    # Calcular GB con precisión decimal
    ...
    while [ $attempt -le $max_attempts ]; do
        if systemctl is-active --quiet redis-server; then
            ...
            if [ $attempt -lt $max_attempts ]; then
                ...
                print_message "Posible problema de configuración. Verificar con:"
...
validate_critical_services() {
    ...
    # Verificar Redis con reintentos
    ...
    for i in {1..3}; do
        if systemctl is-active --quiet redis-server; then
            ...
            print_message "Intento $i/3: Iniciando Redis..."
            systemctl restart redis-server >> $LOG_FILE 2>&1
            sleep 3
        fi
    done
