...
detect_hardware() {
    ...
    # Si RAM es muy pequeña, usar MB para más precisión
    if [ $TOTAL_RAM_GB -eq 0 ]; then
        TOTAL_RAM_MB=$(free -m | awk 'NR==2{print $2}')
        TOTAL_RAM_GB=$(echo "scale=1; $TOTAL_RAM_MB/1024" | bc -l 2>/dev/null || echo "1")
    fi
    
    print_message "RAM Total: ${TOTAL_RAM_GB}GB"
    print_message "RAM Disponible: ${AVAILABLE_RAM_GB}GB"
...
calculate_optimal_settings() {
    ...
    RAM_GB_INT=$(echo "$TOTAL_RAM_GB" | cut -d. -f1)
...
install_optimization_tools_ultra() {
    ...
    # Compilar mozjpeg con fallback
    print_message "Compilando mozjpeg optimizado..."
    cd /tmp
    rm -rf mozjpeg
    if git clone https://github.com/mozilla/mozjpeg.git >> $LOG_FILE 2>&1; then
        cd mozjpeg
        mkdir build && cd build
        if cmake -G"Unix Makefiles" -DCMAKE_INSTALL_PREFIX=/opt/mozjpeg ../ >> $LOG_FILE 2>&1; then
            if make -j$CPU_CORES >> $LOG_FILE 2>&1; then
                ...
                print_warning "mozjpeg compilación falló, usando jpegoptim como fallback"
            fi
        else
            print_warning "mozjpeg cmake falló, usando jpegoptim como fallback"
        fi
    else
        print_warning "mozjpeg download falló, usando jpegoptim como fallback"
    fi
    ...
    print_message "Compilando libavif ultra optimizado..."
    cd /tmp
    rm -rf libavif
    ...
    if git clone --depth 1 https://github.com/AOMediaCodec/libavif.git >> $LOG_FILE 2>&1; then
        ...
        # Configuración ultra optimizada con todos los codecs
        CMAKE_CMD="cmake .. -DCMAKE_BUILD_TYPE=Release \
            ...
            -DAVIF_BUILD_LIBSHARPYUV=ON"
            
        if $CMAKE_CMD >> $LOG_FILE 2>&1; then
            if make -j$CPU_CORES >> $LOG_FILE 2>&1; then
                make install >> $LOG_FILE 2>&1
                ldconfig
                print_success "libavif ultra optimizado compilado exitosamente"
            else
                print_warning "libavif compilación falló, usando versión estándar"
            fi
        else
            print_warning "libavif cmake falló, usando versión estándar"
        fi
    else
        print_warning "libavif download falló, usando versión estándar"
    fi
...
install_image_suites_ultra() {
    ...
    for POLICY_FILE in "${POLICY_FILES[@]}"; do
        if [ -f "$POLICY_FILE" ]; then
            ...
            sed -i 's/<policy domain="resource" name="memory" value="[^"]*"/<policy domain="resource" name="memory" value="'${TOTAL_RAM_GB}'GiB"/g' "$POLICY_FILE"
            sed -i 's/<policy domain="resource" name="map" value="[^"]*"/<policy domain="resource" name="map" value="'$((TOTAL_RAM_GB * 2))'GiB"/g' "$POLICY_FILE"
...
install_redis_ultra() {
    ...
    # Instalar Redis
    print_message "Instalando Redis ultra..."
    apt install -y redis-server redis-tools >> $LOG_FILE 2>&1
    check_result_with_retry "Instalación de Redis"
    
    # Configurar Redis ultra optimizado
    print_message "Configurando Redis ultra para colas masivas..."
    
    # Calcular memoria óptima para Redis
    REDIS_MEMORY=$((TOTAL_RAM_GB * 1024 / 4))  # 25% de RAM total
    [ $REDIS_MEMORY -lt 64 ] && REDIS_MEMORY=64
    [ $REDIS_MEMORY -gt 2048 ] && REDIS_MEMORY=2048
    
    cat << EOF > /etc/redis/redis.conf
# Redis Ultra Configuration for Image Processing Queues
bind 127.0.0.1 ::1
protected-mode yes
port 6379
timeout 300
...
# Configuración de memoria ultra optimizada
maxmemory ${REDIS_MEMORY}mb
maxmemory-policy allkeys-lru
maxmemory-samples 5

# Configuración de persistencia optimizada para queues
save 900 1
save 300 10
save 60 10000
...
tcp-keepalive 300
tcp-backlog $((OPTIMAL_PARALLEL_JOBS * 10))
...
# Configuración de clientes optimizada
maxclients $((OPTIMAL_PARALLEL_JOBS * 20))
...
# Configuraciones específicas para colas de trabajo
...
EOF
    ...
    # Configurar límites del sistema para Redis
    cat << EOF > /etc/systemd/system/redis.service.d/limits.conf
[Service]
LimitNOFILE=1048576
LimitNPROC=131072
EOF
    ...
    print_success "Redis ultra configurado con ${REDIS_MEMORY}MB de memoria"
}
...
[global]
    ...
    memory mode = ram
    page cache size = $((TOTAL_RAM_GB * 32))
    history = $(((TOTAL_RAM_GB + 1) * 3600))
...
EOF
    ...
    sleep 5
    if netstat -tulpn 2>/dev/null | grep -q ":19999"; then
        print_success "Netdata ultra instalado y activo en puerto 19999"
    else
        print_warning "Netdata puede requerir configuración adicional"
    fi
}
...
create_directory_structure_ultra() {
    ...
    mkdir -p /var/www/image-processor/{
        ...
        processed/{jpg,png,webp,avif,gif,svg,tiff},
        temp/{workers,batch,parallel},
        logs/{nginx,php,redis,workers,optimization},
        cache/{thumbnails,metadata,statistics},
        queue/{high,normal,low,failed},
        api/{v1,v2,webhooks},
        config,
        scripts/{optimization,batch,monitoring},
    ...
    chmod 777 /var/www/image-processor/temp
    chmod 777 /var/www/image-processor/uploads
    chmod 777 /var/www/image-processor/cache
    chmod 755 /var/www/image-processor/logs
...
check_service() {
    ...
    if systemctl is-active $1 &> /dev/null; then
        echo -e "${GREEN}✓${NC} $1 activo"
        ((SERVICES_RUNNING++))
        return 0
    else
        echo -e "${RED}✗${NC} $1 inactivo"
        return 1
    fi
}
...
vm.min_free_kbytes = $((TOTAL_RAM_GB * 1024 * 64))
