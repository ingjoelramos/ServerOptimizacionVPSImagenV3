...
install_optimization_tools_ultra() {
    ...
    print_message "Preparando compilación de mozjpeg optimizado..."
    ...
    # Intentar compilación solo si tenemos suficiente RAM y CPU
    if [ $TOTAL_RAM_GB -ge 2 ] && [ $CPU_CORES -ge 2 ]; then
    ...
    print_message "Preparando instalación de libavif optimizado..."
...
install_redis_ultra() {
    ...
    # Detener Redis antes de configurar
    systemctl stop redis-server >> $LOG_FILE 2>&1
    
    # Hacer backup de la configuración original
    if [ -f /etc/redis/redis.conf ]; then
        cp /etc/redis/redis.conf /etc/redis/redis.conf.backup >> $LOG_FILE 2>&1
    fi
    
    # Crear directorios necesarios para Redis
    ...
    # Calcular memoria óptima para Redis (más conservador para sistemas pequeños)
    RAM_GB_INT=$(echo "$TOTAL_RAM_GB" | cut -d. -f1)
    if [ $RAM_GB_INT -le 2 ]; then
...
EOF
    ...
    if systemctl is-active --quiet redis-server; then
        print_success "Redis ultra configurado con ${REDIS_MEMORY}MB de memoria y funcionando"
    else
        print_warning "Redis instalado pero requiere verificación manual"
        print_message "Ejecutar: sudo systemctl status redis-server para verificar"
    fi
}
